...
When a client-side tool runs, the server automatically records a `CLIENT_TOOL_CALL` event span as a child of the current `AGENT_RUN` span. Event spans occur at a point in time (no end time) and mark "this client tool was deferred to the client at this moment in the trace" — the actual execution happens on the client and any child spans/logs from inside it parent themselves under this event span via OTLP/JSON ingest in the next agent run. This happens for every user with no extra configuration.
