...
enum SpanType {
  ...
  /**
   * Client-side tool execution. Parent span lives on the server with
   * status='deferred'; child spans/logs flow back from the client SDK
   * as OTLP/JSON via the ClientToolObservabilityIngest interface in
   * @mastra/observability. See the "Client tools" section in the
   * @mastra/client-js reference for the full flow.
   */
