...
export abstract class BaseObservabilityInstance extends MastraBase implements ObservabilityInstance {
  ...
  /**
   * Internal hook used by external ingest paths (e.g. the client-tool
   * observability ingest in `client-tool/`) to route already-validated
   * events through the normal bus without going through the live span
   * lifecycle. The caller is responsible for constructing well-formed
   * `ExportedSpan`s/`ExportedLog`s and for any validation needed.
   */
  __ingestExternalEvent(event: ObservabilityEvent): void {
    this.emitObservabilityEvent(event);
  }
